...
class ThermalPolicyAllOf(ModelNormal):
    ...
    def _from_openapi_data(cls, *args, **kwargs):  # noqa: E501
        """ThermalPolicyAllOf - a model defined in OpenAPI
        ...
        Keyword Args:
            ...
            fan_control_mode (str): Sets the Fan Control Mode. High Power, Maximum Power and Acoustic modes are supported only on the Cisco UCS C-Series servers and on the X-Series Chassis. Maximum cooling mode is only supported on Unified Edge platforms and Cisco UCS C-Series M7+ servers running firmware 4.3 (5.250001) or later. C480M5 does not support Acoustic mode. * `Balanced` - The fans run faster when needed based on the heat generated by the server. When possible, the fans returns to the minimum required speed. * `LowPower` - The Fans run at the minimum speed required to keep the server cool. * `HighPower` - The fans are kept at higher speed to emphasizes performance over power consumption. This Mode is only supported for UCS X series Chassis. * `MaximumPower` - The fans are always kept at maximum speed. This option provides the most cooling and consumes the most power. This Mode is only supported for UCS X series Chassis. * `Acoustic` - The fan speed is reduced to reduce noise levels in acoustic-sensitive environments. This Mode is only supported for UCS X series Chassis. * `MaximumCooling` - The fan speed is kept for maximum cooling. This mode is only supported for Cisco UCS C-Series server models M7 and above for server firmware 4.3 (5.250001) and later.. [optional] if omitted the server will use the default value of "Balanced"  # noqa: E501
        ...
        class_id = "thermal.Policy" if len(kwargs.get('class_id','').strip()) == 0 else kwargs.get('class_id', "thermal.Policy")
        object_type = "thermal.Policy" if len(kwargs.get('object_type','').strip()) == 0 else kwargs.get('object_type', "thermal.Policy")
