...
class ComputeServerPowerPolicyAllOf(ModelNormal):
    ...
    def _from_openapi_data(cls, *args, **kwargs):  # noqa: E501
        """ComputeServerPowerPolicyAllOf - a model defined in OpenAPI

        Args:

        Keyword Args:
            class_id (str): The fully-qualified name of the instantiated, concrete type. This property is used as a discriminator to identify the type of the payload when marshaling and unmarshaling data.. defaults to "compute.ServerPowerPolicy", must be one of ["compute.ServerPowerPolicy", ]  # noqa: E501
            object_type (str): The fully-qualified name of the instantiated, concrete type. The value should be the same as the 'ClassId' property.. defaults to "compute.ServerPowerPolicy", must be one of ["compute.ServerPowerPolicy", ]  # noqa: E501
            ...
            power_state (str): User configured power state of server. * `Policy` - Power state is set to the default value in the policy. * `PowerOn` - Power state of the server is set to On. * `PowerOff` - Power state is the server set to Off. * `PowerCycle` - Power state the server is reset. * `HardReset` - Power state the server is hard reset. * `Shutdown` - Operating system on the server is shut down. * `Reboot` - Power state of IMC is rebooted.. [optional] if omitted the server will use the default value of "Policy"  # noqa: E501
            server_name (str): The name of the server it is associated with.. [optional]  # noqa: E501
        ...
        class_id = "compute.ServerPowerPolicy" if len(kwargs.get('class_id','').strip()) == 0 else kwargs.get('class_id', "compute.ServerPowerPolicy")
        object_type = "compute.ServerPowerPolicy" if len(kwargs.get('object_type','').strip()) == 0 else kwargs.get('object_type', "compute.ServerPowerPolicy")
