...
class PowerPolicyAllOf(ModelNormal):
    ...
    def _from_openapi_data(cls, *args, **kwargs):  # noqa: E501
        ...
        Keyword Args:
            ...
            allocated_budget (int): Sets the limit for the maximum input power consumption by the chassis (in Watts). Set to 0 for no limit.. [optional] if omitted the server will use the default value of 0  # noqa: E501
            dynamic_rebalancing (str): Sets the dynamic power rebalancing mode of the chassis. If enabled, this mode allows the chassis to dynamically reallocate the power between servers depending on their power usage. * `Enabled` - The power group is administratively enabled. * `Disabled` - The power group is administratively disabled.. [optional] if omitted the server will use the default value of "Enabled"  # noqa: E501
            extended_power_capacity (str): Sets the Extended Power Capacity of the Chassis. If Enabled, this mode allows chassis available power to be increased by borrowing power from redundant power supplies.  This option is only supported for Cisco UCS X series Chassis. * `Enabled` - The power group is administratively enabled. * `Disabled` - The power group is administratively disabled.. [optional] if omitted the server will use the default value of "Enabled"  # noqa: E501
            ...
            power_profiling (str): Sets the Power Profiling of the Server. If Enabled, this field allows the power manager to run power profiling  utility to determine the power needs of the server.  This field is only supported for Cisco UCS X series servers. * `Enabled` - The power group is administratively enabled. * `Disabled` - The power group is administratively disabled.. [optional] if omitted the server will use the default value of "Enabled"  # noqa: E501
