...
class StorageHitachiDisk(ModelComposed):
    ...
    def __init__(self, *args, **kwargs):  # noqa: E501
        ...
        Keyword Args:
            ...
            type_detail (str): Drive type. * `N/A` - Drive Type is not available. * `SAS` - SAS. * `SSD(MLC)` - SSD (MLC). * `SSD(FMC)` - SSD (FMC). * `SSD(FMD)` - SSD (FMD). * `SSD(SLC)` - SSD (SLC). * `SSD` - SSD. * `SSD(RI)` - SSD (RI).. [optional] if omitted the server will use the default value of "N/A"  # noqa: E501
