...
class PartnerintegrationInventoryRelationship(ModelComposed):
    ...
    def _from_openapi_data(cls, *args, **kwargs):  # noqa: E501
        """PartnerintegrationInventoryRelationship - a model defined in OpenAPI
        ...
        Keyword Args:
            ...
            build_status (str): Status of build for inventory collection. * `None` - Default value of the status. i.e. done nothing. * `BackendInProgress` - The backend build is in progress. * `BackendFailed` - The backend build has failed. * `DockerInProgress` - The docker build is in progress. * `DockerFailed` - The docker build has failed. * `UiInProgress` - The UI build is in progress. * `UiFailed` - The UI build has failed. * `ApidocsInProgress` - The apidocs build is in progress. * `ApidocsFailed` - The apidocs build has failed. * `Completed` - The operation completed successfully.. [optional] if omitted the server will use the default value of "None"  # noqa: E501
