...
class WorkflowLoopTaskAllOf(ModelNormal):
    ...
    def __init__(self, *args, **kwargs):  # noqa: E501
        """WorkflowLoopTaskAllOf - a model defined in OpenAPI
        ...
        Keyword Args:
            ...
            count (str): Count value for the loop, this can be a constant or an expression which will evaluate to an integer value. Example, Use the length of the input A which is an array. Count must be less than or equal to 500.. [optional]  # noqa: E501
            loop_start_task (str): Start task where the list of tasks will be executed multiple times based on the count value.. [optional]  # noqa: E501
            ...
            parallel (bool): When set to true the loop will run in parallel else it will run in a serial fashion. Only one task is supported inside the loop task when the loop is run in parallel. Subworkflow can be used inside the single loop task to build complex conditions.. [optional] if omitted the server will use the default value of True  # noqa: E501
        """

        class_id = kwargs.get('class_id', "workflow.LoopTask")
        object_type = kwargs.get('object_type', "workflow.LoopTask")
