...
def lazy_import():
    ...
    globals()['BootSdCardAllOf'] = BootSdCardAllOf


class BootSdCard(ModelComposed):
    ...
    def _from_openapi_data(cls, *args, **kwargs):  # noqa: E501
        """BootSdCard - a model defined in OpenAPI

        Keyword Args:
            class_id (str): The fully-qualified name of the instantiated, concrete type. This property is used as a discriminator to identify the type of the payload when marshaling and unmarshaling data.. defaults to "boot.SdCard", must be one of ["boot.SdCard", ]  # noqa: E501
            object_type (str): The fully-qualified name of the instantiated, concrete type. The value should be the same as the 'ClassId' property.. defaults to "boot.SdCard", must be one of ["boot.SdCard", ]  # noqa: E501
            ...
            subtype (str): The subtype for the selected device type. * `None` - No sub type for SD card boot device. * `flex-util` - Use of FlexUtil (microSD) card as sub type for SD card boot device. * `flex-flash` - Use of FlexFlash (SD) card as sub type for SD card boot device. * `SDCARD` - Use of SD card as sub type for the SD Card boot device.. [optional] if omitted the server will use the default value of "None"  # noqa: E501
        ...
        class_id = "boot.SdCard" if len(kwargs.get('class_id','').strip()) == 0 else kwargs.get('class_id', "boot.SdCard")
        object_type = "boot.SdCard" if len(kwargs.get('object_type','').strip()) == 0 else kwargs.get('object_type', "boot.SdCard")
