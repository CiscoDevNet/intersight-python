...
class NetworkElement(ModelComposed):
    ...
    def _from_openapi_data(cls, *args, **kwargs):  # noqa: E501
        ...
        Keyword Args:
            ...
            inter_cluster_link_state (str): The intercluster link state of the switch. * `Unknown` - The link operational state is not known. * `Up` - The link operational state is up. * `Down` - The link operational state is down. * `Degraded` - The link is degraded but operational. This state is applicable to port channels when any one of the member links is down.. [optional] if omitted the server will use the default value of "Unknown"  # noqa: E501
