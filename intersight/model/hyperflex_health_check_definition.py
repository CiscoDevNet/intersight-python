...
class HyperflexHealthCheckDefinition(ModelComposed):
    ...
    def __init__(self, *args, **kwargs):  # noqa: E501
        ...
        Keyword Args:
            ...
            supported_hypervisor_type (str): Hypervisor type that the Health Check is supported on (All, if it is hypervisor agnostic). * `All` - The Health Check is hypervisor-agnostic. * `ESXi` - The Health Check is supported only on Vmware ESXi hypervisor of any version. * `IWE` - The Health Check is supported only on Cisco IWE platform. * `HyperV` - The Health Check is supported only on Microsoft HyperV hypervisor.. [optional] if omitted the server will use the default value of "All"  # noqa: E501
